...
void KGrRunner::getRules()
{
    ...
    // if (spriteId < 1) {
        // qDebug() << "pointsPerCell" << pointsPerCell
                                 ...
                                 AnimationType & anim, int & interval)
{
    ...
    // if (spriteType == HERO) {
        // qDebug() << "Calling standOnEnemy() for" << gridX << gridY;
    // }
...
KGrHero::KGrHero (KGrLevelPlayer * pLevelPlayer, KGrLevelGrid * pGrid,
    ...
    nuggets (0)
{
    // qDebug() << "THE HERO IS BORN at" << i << j << "sprite ID" << pSpriteId;
    rules->getHeroTimes (runTime, fallTime, enemyFallTime, trapTime);
    // qDebug() << "Hero run time" << runTime << "fall time" << fallTime;
    interval = runTime;
}
...
bool KGrHero::dig (const Direction diggingDirection, int & i, int & j)
{
    ...
    if ((grid->cellType  (gridI + relativeI, gridJ + 1) == BRICK) &&
		...
		// If cannot move to next cell and stand, is an enemy under it?
                // qDebug() << "Calling standOnEnemy() at gridX" << gridX
            ...
            if ((! canStand) && (! enemyUnder)) {
                qDebug() << "INVALID DIG: hero at" << gridI << gridJ
    ...
    prevInCell (-1)
{
    ...
    interval      = runTime;
    // qDebug() << "ENEMY" << pSpriteId << "IS BORN at" << i << j;
    // if (pSpriteId < 2) {
        // qDebug() << "Enemy run time " << runTime << "fall time" << fallTime;
        // qDebug() << "Enemy trap time" << trapTime << "Rules type" << rulesType;
    // }
    t.start(); // IDW
}
